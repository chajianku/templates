...
if (isset($i['mode'][2]) && $i['mode'][2] == 'sign') {
	...
	<div class="row">
		<div class="col s12">
			<div class="container">
				<div class="card">
					<div class="card-image black" style="height:50px;padding:0px;">
						<span style="padding:10px;" class="card-title">程序设置</span>
					</div>
					<div class="card-content">
								...
								<table class="table table-hover">
									...
									<tbody>
										...
										<tr><td>签到模式设置<br/>选择多个将在某个模式失败后使用下一种<br/>启用的签到模式越多，消耗的流量和时间越多</td><td><?php $sign_mode = unserialize(option::get('sign_mode')) ?>
											<input type="checkbox" id="client" name="sign_mode[]" value="1" <?php if(!empty($sign_mode) && in_array('1',$sign_mode)) { echo 'checked'; } ?>> <label for="client">模拟手机客户端签到</label><br/>
											<input type="checkbox" id="mobile" name="sign_mode[]" value="3" <?php if(!empty($sign_mode) && in_array('3',$sign_mode)) { echo 'checked'; } ?>> <label for="mobile">手机网页签到</label><br/>
											<input type="checkbox" id="pc" name="sign_mode[]" value="2" <?php if(!empty($sign_mode) && in_array('2',$sign_mode)) { echo 'checked'; } ?>> <label for="pc">网页签到</label>
										</td>
									</tr>
									<tr><td>贴吧数据分表<br/><br/>全部留空为不分表<br/>每行一个表名，无需填写表前缀<br/>错误的设置将导致签到程序不能正常工作<br/>当某一表存储的贴吧记录数目明显超过设定值时才能生效<br/>单个用户将终生使用某一表，所以请设置小点<br/>当所有的表的记录都超过设定值时，新的贴吧将往最后一个表写</td><td>
										<div class="input-field s12">
											<input type="number" min="0" step="1" class="form-control" id="fb" name="fb" value="<?php echo option::get('fb') ?>"><label for="fb">记录超过此行时分表</label>
										</div><br/>
						...
						<button type="submit" class="btn blue">保存设置=w=<i class="mdi-content-send right"></i></button>
					</form>
				</div>
			</div>
		</div>
		...
		<div class="row">
			<div class="col s12">
						...
						<div class="card-content">
							...
							<form action="setting.php?mod=admin:set&type=system" method="post">
								<div class="table-responsive">
									...
									?>
									<tr><td>注册杂项设置</td><td>
										<input type="checkbox" name="enable_reg" value="1" <?php if(option::get('enable_reg') == 1) { echo 'checked'; } ?> id="enable_reg"> <label for="enable_reg">允许用户注册</label><br/>
										<input type="checkbox" name="protect_reg" value="1" <?php if(option::get('protect_reg') == 1) { echo 'checked'; } ?> id="protect_reg"> <label for="protect_reg">反恶意注册</label>
									</td>
								</tr>
								...
								<div class="input-field s12">
									<label for="sent-email">发件人邮箱</label>
									<input id="sent-email" type="email" name="mail_name" class="form-control"  value="<?php echo option::get('mail_name') ?>">
								</div><br/>

								<div class="input-field s12">
									<label for="sent-name">发件人邮箱</label>
									<input id="sent-name" type="text" name="mail_yourname" class="form-control" value="<?php echo option::get('mail_yourname') ?>" >
								</div><br/>

								<div id="smtp_set" <?php if(option::get('mail_mode') != 'SMTP') { echo 'style="display:none;"'; } ?>>
									<div class="input-field s12">
										<label for="smtp-add">SMTP服务器地址</label>
										<input id="smtp-add" type="text" name="mail_host" class="form-control"  value="<?php echo option::get('mail_host') ?>">
									</div><br/>
									<div class="input-field s12">
										<label for="smtp-port">SMTP服务器端口</label>
										<input id="smtp-port" type="number" name="mail_port" class="form-control"  value="<?php echo option::get('mail_port') ?>">
									</div><br/>
									<div class="input-group">
								...
								<div id="smtp_set_auth" <?php if(option::get('mail_auth') == '0') { echo 'style="display:none;"'; } ?>>
									<div class="input-field s12">
										<label for="smtp-user">SMTP 用户名</label>
										<input id="smtp-user" type="text" name="mail_smtpname" class="form-control" value="<?php echo option::get('mail_smtpname') ?>">
									</div><br/>

									<div class="input-field s12">
										<label for="smtp-pw">SMTP 密码</label>
										<input id="smtp-pw" type="password" name="mail_smtppw" class="form-control" value="<?php echo option::get('mail_smtppw') ?>">
									</div><br/>
								</div>
							</div>
				...
				<iframe style="display:none;" src="https://www.imim.pw/api/stat.php?title=<?php echo SYSTEM_NAME; ?>&url=<?php $site='http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']; preg_match("/(.*?)index.php/", $site,$result); echo $result[1];?>&version=material-template"></iframe>
				<?php
